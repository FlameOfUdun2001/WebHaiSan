...
router.post('/them.html', Passport.requireAuth, upload.single('hinh'), async (req, res) => {
  ...
  req.checkBody('price', 'Price phải là số').isInt();

  req.checkBody('description', 'Description không được để trống').notEmpty();
  ...
  req.flash('response_message', 'Thêm sản phẩm thành công !');
...
router.post('/sua/:id.html', upload.single('hinh'), async (req, res) => {
  ...
  req.flash('response_message', 'Sửa sản phẩm thành công !');
...
router.get('/xoa/:id', Passport.requireAuth, async (req, res) => {
  ...
  if (!docProduct || !docProduct.id) {
    ...
    req.flash('response_message', 'Xóa sản phẩm thành công !');
  }
